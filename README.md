...
Fixes Kurdish character **rendering AND input** (`ە ڕ ێ ۆ ڵ ڤ`) in Tkinter apps.

Solves the Tcl/Tk 8.6 bug on Windows where Kurdish keyboard input is converted to `?` characters.

```python
from kurdish_sorani_tools import KurdishTkinter, KURDISH_CHARS, KURDISH_FONTS
...
root = tk.Tk()
KurdishTkinter.fix_window(root)  # Fix fonts for display

# ═══ Fix Kurdish keyboard INPUT (critical on Windows!) ═══
# Without this, typing Kurdish shows '????' in Entry/Text widgets
entry = tk.Entry(root)
entry.pack()
KurdishTkinter.fix_input(entry)  # ← Fixes Kurdish typing!

# Or use the shortcut that does BOTH (font + input fix):
entry = KurdishTkinter.create_entry(root, width=30)
entry.pack()

# Works with Text widgets too:
text = KurdishTkinter.create_text(root, width=40, height=5)
text.pack()

# ═══ Fix input on ANY existing widget ═══
my_entry = tk.Entry(root, font=("Tahoma", 14))
my_entry.pack()
KurdishTkinter.fix_input(my_entry)  # Just add this one line!

# ═══ Other helpers ═══
label = KurdishTkinter.create_label(root, text="سڵاو جیهان")
label.pack()

button = KurdishTkinter.create_button(root, text="کلیک بکە")
button.pack()

# Get the best font name for Kurdish
font_name, size = KurdishTkinter.get_best_font(14)
print(font_name)  # Tahoma (or best available on system)

# Quick message popup
KurdishTkinter.display_message("سڵاو! ئەمە پەیامێکی کوردییە")

# Access character/font lists
print(KURDISH_CHARS)  # ['ئ', 'ا', 'ب', 'پ', 'ت', ...]
print(KURDISH_FONTS)  # ['Tahoma', 'Arial Unicode MS', ...]

root.mainloop()
```

**Why is this needed?** Tcl/Tk 8.6 on Windows misidentifies the Kurdish keyboard layout (maps it to Cyrillic) and sends `?` (ASCII 63) instead of the actual Kurdish Unicode characters. `KurdishTkinter.fix_input()` uses the Windows `ToUnicodeEx` API to intercept keystrokes and get the real characters directly from the OS.
